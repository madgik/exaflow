...
class FederatedKMeans:
    ...
    def fit(self, x, *, feature_names=None):
        ...
        global_invalid = float(self.agg_client.sum([local_invalid])[0])
    ...
    def _fit_one_initialization(
        ...
        *,
        ...
        final_sum_local, final_count_local = self._compute_local_sums_and_counts(
            X=X,
            labels=labels,
            n_features=n_features,
        )
        final_sum_global_arr = self.agg_client.sum(final_sum_local.ravel())
        final_count_global_arr = self.agg_client.sum(final_count_local)
        final_sum_global = np.asarray(final_sum_global_arr, dtype=float).reshape(
            (self.n_clusters, n_features)
        )
        final_count_global = np.asarray(final_count_global_arr, dtype=float)
        final_centers = self._compute_centers_from_sums_and_counts(
            sums=final_sum_global,
            counts=final_count_global,
            n_features=n_features,
            like=centers,
        )
        if converged:
            converged = np.linalg.norm(final_centers - centers, ord="fro") <= self.tol
