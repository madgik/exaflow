...
from tests.standalone_tests.federated_algorithms.utils.federated_algorithm_test import FederatedAlgorithmTest
...
class TestNumpyAggregatorFed(FederatedAlgorithmTest):
    ...
    def compare(self, federated_output, centralized_output, **kwargs):
        ...
        if method == "fed_union":
            if isinstance(federated_output, np.ndarray):
                assert np.array_equal(np.sort(federated_output), np.sort(centralized_output))
            else:
    ...
    def test_fed_weighted_avg(self, n_workers):
        ...
        self.run_comparison(X=X, y=np.zeros(n_workers), n_workers=n_workers, method="fed_weighted_avg", weights=weights)

    @pytest.mark.parametrize("n_workers", [1, 3])
    def test_fed_union_numeric(self, n_workers):
        X = np.array([[1.0, 2.0], [2.0, 3.0], [3.0, 4.0]])[:n_workers]
        self.run_comparison(X=X, y=np.zeros(n_workers), n_workers=n_workers, method="fed_union")
...
class TestNumpyAggregatorGlobal(FederatedAlgorithmTest):
    ...
    def compare(self, federated_output, centralized_output, **kwargs):
        ...
        assert np.allclose(federated_output, centralized_output, atol=1e-6, equal_nan=True)

    @pytest.mark.parametrize("method", ["global_sum", "global_count", "global_avg", "global_min", "global_max"])
    def test_federated_algorithm_with_one_worker(self, method):
