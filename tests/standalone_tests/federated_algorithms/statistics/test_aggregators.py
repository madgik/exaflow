    ...
    def compute_centralized_result(self, X, y, **kwargs):
        ...
        if method == "fed_union":
            if np.issubdtype(X.dtype, np.number):
                ...
                return (
                    np.concatenate([unique_vals, [np.nan]]) if has_nan else unique_vals
                )
            else:
                has_none = bool(np.any(X == None))  # noqa: E711
                unique_vals = np.unique(X[X != np.array(None)])
                return (
                    np.concatenate([unique_vals, [None]]) if has_none else unique_vals
                )
        elif method == "fed_sum":
            ...
            return np.where(valid_count == 0, np.nan, np.sum(filled, axis=0) / valid_count)
