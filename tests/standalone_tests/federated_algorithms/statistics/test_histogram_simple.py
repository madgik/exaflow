...
from exaflow.algorithms.federated.statistics.histogram_simple import SimpleHistogram
...
class TestSimpleHistogram(FederatedAlgorithmTest):
    ...
    def compute_federated_result(self, X, y, *, agg_client, **kwargs):
        """Compute the federated histogram using the StandardHistogram algorithm."""
    ...
    def test_federated_algorithm_with_one_worker(self, case):
        ...
        if case["name"] == "all_nans":
            with pytest.raises(BadInputError, match="No finite data"):
                self.run_comparison(
