...
def test_kmeans_cluster_creator_uses_base_transform_data_and_metadata():
    assert "transform_data_and_metadata" not in KMeansClusterCreator.__dict__
...
def test_kmeans_cluster_creator_preprocessing_feeds_plain_downstream_udf(monkeypatch):
    from exaflow.worker import config as worker_config
    ...
    def plain_downstream_udf(data):
        return {
            "n_rows": len(data),
            "n_clusters": int(data["kmeans_cluster"].nunique()),
        }

    result = udf_service._execute_udf(
        udf=plain_downstream_udf,
        kw_args={},
        data=transformed,
    ...
    assert transformed is data
    assert transformed_metadata["kmeans_cluster"]["is_categorical"] is True
    assert result == {"n_rows": 9, "n_clusters": 3}
