...
- **Wilkinson** (default) — produces "nice-number" bin boundaries for numerical
  targets, chosen so that bin edges are easy to read. Selected via
  `histogram_type = "wilkinson"`.
- **Simple** — equal-width bins for numerical targets between the global min
  and max. Selected via `histogram_type = "simple"`.
...
- `histogram_type`: `"wilkinson"` or `"simple"`, default `"wilkinson"`. Ignored
...
[Histogram (SQL)](../../exaflow/algorithms/exareme3/statistics/histogram_sql.py)
